...
public class CommandBatchPreparer : ICommandBatchPreparer
{
    ...
    private static bool IsModified(IReadOnlyList<IColumn> columns, IReadOnlyModificationCommand command)
    {
        ...
        {
            (IProperty, IUpdateEntry) firstProperty = default;
            List<(IProperty, IUpdateEntry)>? sharingProperties = null;
            foreach (var entry in command.Entries)
            {
                ...
                if (property != null)
                {
                    ...
                    if ((entry.EntityState != EntityState.Modified && property.GetAfterSaveBehavior() == PropertySaveBehavior.Save))
                    {
                        return true;
                    }
                    firstProperty = (property, entry);
                }
            }
        }
